...
namespace App\Services\Qoyod\Presenters\Notes\DebitNotes;

use App\Services\Qoyod\Contracts\Resources\VendorResourceInterface;
use Illuminate\Support\Facades\Cache;

class DebitNotePresenter
{
    protected int $ttlMinutes;

    public function __construct(protected VendorResourceInterface $vendors)
    {
        $this->ttlMinutes = config('qoyod.cache_ttl', 10);
    }


    // get all Credit Note with customer name
    public function getDebitNoteWithVendorName(array $rows): array
    {
        try {
