...
class EmployeesDataTable extends ArabicSearchDataTable
{
    ...
    */
    ...
    protected function getColumns(): array
    {
        return [
            ...
            ['data' => 'business_card',     'name' => 'business_card',      'title' => 'بطاقة العمل '],
    ...
    */
    protected function addCustomColumns($datatable)
    {
        $datatable
            ...
            ->addColumn('business_card', function ($row) {
                return $this->routeName(route('hr.employees.business-card', $row->id), 'عرض البطاقة');
            })
            ...
            ->addColumn('actions', function ($row) {
                return (auth()->user()->can('تعديل موظف') || auth()->user()->can('حذف موظف') || auth()->user()->can('إعادة تعين كلمة المرور')) ? view('hr.employees.action', ['row' => $row, 'route' => $this->route])->render() : ' ليس لديك صلاحيات';
            });
    }
    ...
    {
        return ['checkbox', 'profile_img', 'name', 'business_card', 'fingerprints', 'actions'];
    }
    /*
    |============================================================================
    |                               Optional Methods
    ...
    */
    ...
    {
        if (auth()->user()->can('إضافة موظف')) {
            return [
                [
                    'text'      => '<i class="fas fa-plus-circle me-1"></i> إضافة موظف',
