<!-- resources/views/judicial_affairs/lawsuits/sections/requests.blade.php -->
    ...
    <div class="card card-action mb-6">
        <div class="card-header align-items-center bg-white border-bottom mb-5">
            <h5 class="card-action-title mb-0 text-primary fw-bold">
                <i class="ti ti-list-check ti-lg me-1_5 text-body me-3 text-primary fw-bold"></i> الطلبات
            </h5>
        </div>
        <div class="card-body pt-3">
            <form id="requestsFormId" method="POST"
                action="{{ route('legal-affairs.lawsuits.lawsuit-section.requests', $lawsuit->id) }}"
                enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row g-4">
                    <div class="col-12">
                        <label for="request" class="form-label text-primary fw-bold fs-6">النص</label>
                        <textarea id="request" name="request" class="form-control @error('request') is-invalid @enderror" rows="4"
                            placeholder="أدخل الطلبات">{{ old('request', $lawsuit->request) }}</textarea>
                        @error('request')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-12">
                        <label for="request_attachment" class="form-label text-primary fw-bold fs-6">المرفق</label>
                        <input type="file" id="request_attachment" name="request_attachment"
                            class="form-control @error('request_attachment') is-invalid @enderror">
                        @error('request_attachment')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        @if ($lawsuit->request_attachment)
                            <div class="mt-2 d-flex align-items-center">
                                ...
                                <!-- زر حذف المرفق -->
                                <a class="btn btn-outline-danger btn-sm delete-attachment-request text-danger"
                                    data-id="{{ $lawsuit->id }}">
...
<script>
    $(document).on('click', '.delete-attachment-request', function() {
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#request').val('');
                        $('.delete-attachment-request').closest('div').remove();
    ...
    $(document).ready(function() {
        $('#requestsFormId').on('submit', function(e) {
                ...
                success: function(response) {
                    if (response.success) {
                        if (response.message === "لم يتم إجراء أي تغييرات.") {
                            ...
                            if (response.request_attachment) {
                                ...
                                var attachmentButtons = `
                                            <div class="mt-2 d-flex align-items-center">
                                                ...
                                                <!-- زر حذف المرفق -->
                                                <a class="btn btn-outline-danger btn-sm delete-attachment-request text-danger" data-id="{{ $lawsuit->id }}">
                                                    <i class="fas fa-trash-alt me-1"></i> حذف
                                                </a>
                                            </div>
                                        `;
                                ...
                                $('.delete-attachment-request').closest('div').remove();
                                $('#request_attachment').closest('div').append(
                                    attachmentButtons);
                            } else {
