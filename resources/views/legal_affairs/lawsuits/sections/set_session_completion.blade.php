...
@section('title', 'استكمال ضبط الجلسة ')
...
<!-- Page Scripts -->
@section('page-script')
    @vite(['resources/assets/js/form-wizard-validation-set_session.js'])
    <script></script>
        ...
        <div class="col-12">
            <div id="wizard-validation" class="bs-stepper mt-2">
                ...
                <div class="bs-stepper-content">
                    ...
                    @endif
                    <form id="power-form" action="{{ route('sessions.set_session_completion.update', $session->id) }}"
                            ...
                            <div class="row g-6">
                                ...
                                <div class="col-md-6 mb-4 mt-4">
                                    ...
                                    <select id="summary_report_status" name="summary_report_status"
                                        class="form-select select2" required data-placeholder="اختر حالة التقرير الإجمالي">
                                        <option value=""></option>
                                        <option value="مرفوعة للنظر"
                                            {{ old('summary_report_status', $session->summary_report_status) == 'مرفوعة للنظر' ? 'selected' : '' }}>
                                            مرفوعة للنظر</option>
                                        <option value="لا يوجد"
                                            {{ old('summary_report_status', $session->summary_report_status) == 'لا يوجد' ? 'selected' : '' }}>
                                            لا يوجد</option>
                                        <option value="شطب الدعوى"
                                            {{ old('summary_report_status', $session->summary_report_status) == 'شطب الدعوى' ? 'selected' : '' }}>
                                            شطب الدعوى</option>
                                        <option value="الصلح"
                                            {{ old('summary_report_status', $session->summary_report_status) == 'الصلح' ? 'selected' : '' }}>
                                            الصلح</option>
                                        <option value="وقف السير"
                                            {{ old('summary_report_status', $session->summary_report_status) == 'وقف السير' ? 'selected' : '' }}>
                                            وقف السير</option>
                                        <option value="تأجيل"
                                            {{ old('summary_report_status', $session->summary_report_status) == 'تأجيل' ? 'selected' : '' }}>
                                            تأجيل</option>
                                        <option value="حفظ الدعوى"
                                            {{ old('summary_report_status', $session->summary_report_status) == 'حفظ الدعوى' ? 'selected' : '' }}>
                                            حفظ الدعوى</option>
                                        <option value="حكم موضوعي"
                                            {{ old('summary_report_status', $session->summary_report_status) == 'حكم موضوعي' ? 'selected' : '' }}>
                                            حكم موضوعي</option>
                                        <option value="حكم شكلي"
                                            {{ old('summary_report_status', $session->summary_report_status) == 'حكم شكلي' ? 'selected' : '' }}>
                                            حكم شكلي</option>
                                        <option value="تأييد الحكم"
                                            {{ old('summary_report_status', $session->summary_report_status) == 'تأييد الحكم' ? 'selected' : '' }}>
                                            تأييد الحكم</option>
                                    </select>
                                </div>

                                <div class="col-md-6 mb-4 mt-4" id="last_objection_deadline_container"
                                ...
                                <div class="col-md-6 mb-4 mt-4" style="display: none" id="rule_type_container">
                                    <label for="rule_type" class="form-label"> نوع الحكم </label>
                                    <select id="rule_type" name="rule_type" class="form-select select2" required
                                ...
                                <div class="col-md-6 mb-4 mt-4" style="display: none" id="execution_format_container">
                                    <label class="form-label">صيغة تنفيذية</label>

                                    <div class="mt-5">
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" id="execution_format_yes"
                                                name="execution_format" value="نعم"
                                                {{ old('execution_format', $session->execution_format) == 'نعم' ? 'checked' : '' }}>
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" id="execution_format_no"
                                                name="execution_format" value="لا"
                                                {{ old('execution_format', $session->execution_format) == 'لا' ? 'checked' : '' }}>
                                ...
                                <div class="col-md-6 mb-4 mt-4" id="expected_execution_date_container"
                                    style="display: none; position: relative">
                                ...
                                <div class="col-md-6 mb-4 mt-4">
                                    ...
                                    <input type="number" id="execution_minutes" name="execution_minutes"
                                        class="form-control"
                                        value="{{ old('execution_minutes', $session->execution_minutes) }}" required />
                                </div>

                                <!-- الملاحظات -->
                                <div class="col-12 mb-4 mt-4">
                                    <label for="notes" class="form-label">الملاحظات</label>
                                    <textarea id="notes" name="notes" class="form-control" rows="5" required>{{ old('notes', $session->notes) }}</textarea>
                                </div>
                        ...
                        <div id="additional-info" class="content">
                            <div class="row g-6">
                                ...
                                <div class="col-md-6 mb-4 mt-4" id="rule_attached_container" style="display: none">
                                    <label for="rule_attached" class="form-label"> مرفق الحكم</label>
                                    <input type="file" id="rule_attached" required name="rule_attached"
                                        class="form-control" />
