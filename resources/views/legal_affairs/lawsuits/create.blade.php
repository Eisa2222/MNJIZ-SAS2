...
@section('title', 'إضافة دعوى ')

@section('breadcrumb')
    <li><a href="#">الشؤون القانونية</a></li>
    <li><a href="{{ route('legal-affairs.lawsuits.index') }}"> الدعاوى</a></li>
    <li class="breadcrumb-item d-flex align-items-center"><a href="#">إضافة دعوى </a>
        <i class="ti ti-star favorite-icon" data-page-name="إضافة دعوى " data-page-url="{{ url()->current() }}"
        ...
        <div class="col-12">
            <div id="wizard-validation" class="bs-stepper mt-2">
                ...
                <div class="bs-stepper-content">
                    ...
                    @endif
                    <form id="power-form" action="{{ route('legal-affairs.lawsuits.store') }}" method="POST"
                        ...
                        <div id="basic-info" class="content  dstepper-block">
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" id="name" name="name" placeholder="اسم الدعوى"
                                        class="form-control" value="{{ old('name') }}" required />
                                </div>

                                <div class="col-md-6">
                                    <label for="lawsuit_number" class="form-label">رقم الدعوى </label>
                                    <input type="number" id="lawsuit_number" name="lawsuit_number" placeholder="رقم الدعوى"
                                        class="form-control" value="{{ old('lawsuit_number') }}" required />
                                </div>
                                        ...
                                        <optgroup label="العملاء">
                                            @foreach ($mergedList->where('thisType', 'customer') as $customer)
                                                <option value="{{ $customer->id }}-customer"
                                                    data-content="<i class='ti ti-rosette-discount-check-filled text-success' title='عميل'></i> {{ $customer->name }}"
                                                    {{ in_array($customer->id . '-customer', old('plaintiff_id', [])) ? 'selected' : '' }}>
                                                    {{ $customer->name }}
                                                </option>
                                            @endforeach
                                        </optgroup>
                                        <optgroup label="الخصوم">
                                            @foreach ($mergedList->where('thisType', 'opponent') as $opponent)
                                                <option value="{{ $opponent->id }}-opponent"
                                                    ...
                                                    {{ in_array($opponent->id . '-opponent', old('plaintiff_id', [])) ? 'selected' : '' }}>
                                                    {{ $opponent->name }}
                                                </option>
                                            @endforeach
                                        </optgroup>
                                    ...
                                    <select id="defendant_id" name="defendant_id[]" class="w-100 select2"
                                        ...
                                        <optgroup label="العملاء">
                                            @foreach ($mergedList->where('thisType', 'customer') as $customer)
                                                <option value="{{ $customer->id }}-customer"
                                                    data-content="<i class='ti ti-rosette-discount-check-filled text-success' title='عميل'></i> {{ $customer->name }}"
                                                    {{ in_array($customer->id . '-customer', old('defendant_id', [])) ? 'selected' : '' }}>
                                                    {{ $customer->name }}
                                                </option>
                                            @endforeach
                                        </optgroup>
                                        <optgroup label="الخصوم">
                                            @foreach ($mergedList->where('thisType', 'opponent') as $opponent)
                                                <option value="{{ $opponent->id }}-opponent"
                                                    data-content="<i class='ti ti-rosette-discount-check-filled text-danger' title='خصم'></i> {{ $opponent->name }}"
                                                    {{ in_array($opponent->id . '-opponent', old('defendant_id', [])) ? 'selected' : '' }}>
                                                    {{ $opponent->name }}
                                                </option>
                                            @endforeach
                                        </optgroup>
                                    ...
                                    <select id="category_id" name="category_id" class="form-select select2" required
                                        ...
                                        @foreach ($mainCategories as $mainCategory)
                                            <option value="{{ $mainCategory->id }}"
                                                {{ old('category_id') == $mainCategory->id ? 'selected' : '' }}>
                                                {{ $mainCategory->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="position-relative">
                                        <select id="subcategory_id" name="subcategory_id" class="form-select select2"
                                            required data-placeholder="اختر التصنيف الفرعي" disabled>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="position-relative">
                                        <select id="lawsuit_type_id" name="lawsuit_type_id" class="form-select select2"
                                            required data-placeholder="اختر نوع الدعوى" disabled>
                                            <option value="">اختر التصنيف الفرعي أولاً</option>
                                        </select>
                                        <!-- Loader لأنواع الدعاوى -->
                                    ...
                                    <select id="power_of_attorney_id" name="power_of_attorney_id[]" class="w-100 select2"
                                        ...
                                        @foreach ($power_of_attorneys as $power_of_attorney)
                                            <option value="{{ $power_of_attorney->id }}"
                                                {{ in_array($power_of_attorney->id, old('power_of_attorney_id', [])) ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select id="assigned_to" name="assigned_to[]" class="w-100 select2"
                                        ...
                                        data-placeholder="اختر فريق الدعوى" disabled>
                                    </select>
                                </div>

                                <div class="col-md-6">
                                    <label for="settings_main_courts" class="form-label">المحكمة </label>
                                    <select id="settings_main_courts" name="main_courts_id" class="form-select select2"
                                        required data-placeholder="اختر المحكمة  ">
                                        <option value=""></option>
                                        @foreach ($settings_main_courts as $court)
                                            <option value="{{ $court->id }}"
                                                {{ old('main_courts_id') == $court->id ? 'selected' : '' }}>
                                                {{ $court->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <select id="regions_id" name="regions_id" class="form-select select2" required
                                        ...
                                        @foreach ($settings_regions as $region)
                                            <option value="{{ $region->id }}"
                                                {{ old('regions_id') == $region->id ? 'selected' : '' }}>
                                                {{ $region->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div id="additional-info" class="content">
                            <div class="row g-3">
                                ...
                                {{-- <div class="col-md-6">
                                    <label for="lawsuit_attachment" class="form-label">صحيفة الدعوى</label>
                                    <textarea id="lawsuit_attachment" name="lawsuit_attachment" class="form-control" rows="4">{{ old('lawsuit_attachment') }}</textarea>
                                </div> --}}

                                <div class="col-md-6">
                                    <label for="lawsuit_subject" class="form-label">موضوع الدعوى</label>
                                    <textarea id="lawsuit_subject" name="lawsuit_subject" class="form-control" rows="4">{{ old('lawsuit_subject') }}</textarea>
                                </div>

                                <div class="col-md-6">
                                    <label for="plaintiff_requests" class="form-label">طلبات المدعي</label>
                                    <textarea id="plaintiff_requests" name="plaintiff_requests" class="form-control" rows="4">{{ old('plaintiff_requests') }}</textarea>
                                </div>

                                <div class="col-md-12">
                                    <label for="lawsuit_proofs" class="form-label">أسانيد الدعوى</label>
                                    <textarea id="lawsuit_proofs" name="lawsuit_proofs" class="form-control" rows="4">{{ old('lawsuit_proofs') }}</textarea>
                                </div>
