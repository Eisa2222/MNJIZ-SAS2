...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item d-flex align-items-center"><a href="#"> تعديل جلسة </a>
        <i class="ti ti-star favorite-icon" data-page-name="تعديل جلسة " data-page-url="{{ url()->current() }}"
...
@section('content')
        ...
        <div class="col-12">
            <div id="wizard-validation" class="bs-stepper mt-2">
                ...
                <div class="bs-stepper-content">
                    ...
                    @endif
                    <form id="form" action="{{ route('legal-affairs.sessions.update', $session->id) }}" method="POST"
                            ...
                            <div class="row g-3">
                                    ...
                                    <select id="project_id" name="project_id" class="form-select select2" required
                                        ...
                                        @foreach ($projects as $project)
                                            <option value="{{ $project->id }}"
                                                {{ old('project_id', $session->lawsuit->project_id) == $project->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="position-relative">
                                        <select id="lawsuit_id" name="lawsuit_id" class="form-select select2" required
                                            ...
                                            @if (isset($session->lawsuit))
                                                <option value="{{ $session->lawsuit->id }}" selected>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" id="session_name" disabled name="session_name"
                                        class="form-control" value="{{ old('session_name', $session->session_name) }}"
                                        readonly placeholder="سيتم إنشاؤه تلقائياً" />
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="position-relative">
                                        <select id="assigned_to" name="assigned_to[]" class="w-100 selectpicker"
                                            ...
                                            data-live-search="true" data-placeholder="اختر المكلفين">
                                            @if (isset($session->assignedUsers) && $session->assignedUsers->count() > 0)
                                    ...
                                    <select id="entity_ranks_id" name="entity_ranks_id" class="form-select select2"
                                        ...
                                        @foreach ($settings_entity_ranks as $ranks)
                                            <option value="{{ $ranks->id }}"
                                                {{ old('entity_ranks_id', $session->entity_ranks_id) == $ranks->id ? 'selected' : '' }}>
                                                {{ $ranks->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-3" style="position: relative">
                                    ...
                                    <input type="text" id="session_date" name="session_date"
                                        ...
                                        value="{{ old('session_date', $session->hijri_session_date) }}" required
    ...
    <script>
        $(document).ready(function() {
            // بيانات الجلسة الحالية للتهيئة الأولية
            const currentProjectId = '{{ $session->lawsuit->project_id ?? '' }}';
            const currentLawsuitId = '{{ $session->lawsuit_id ?? '' }}';
            const currentAssignedUsers = @json($session->assignedUsers->pluck('id')->toArray() ?? []);
            ...
            // تهيئة المكلفين في حالة وجود بيانات
            if (currentProjectId && currentLawsuitId && currentAssignedUsers.length > 0) {
                ...
                // إظهار عدد المكلفين
                $('#employees-count').text(`${currentAssignedUsers.length} موظف`)
            ...
            $('#project_id').on('change', function() {
                ...
                sessionNameInput.val(''); // تفريغ اسم الجلسة
                lawsuitsCount.hide();
                employeesCount.hide();
                        ...
                        success: function(response) {
                            ...
                            if (response.lawsuits && response.lawsuits.length > 0) {
                                $.each(response.lawsuits, function(index, lawsuit) {
                                    const isSelected = lawsuit.id == currentLawsuitId &&
                                        projectId == currentProjectId;
                                    lawsuitSelect.append(
                                        `<option value="${lawsuit.id}" ${isSelected ? 'selected' : ''}>${lawsuit.name}</option>`
                                    );
                                });
                                ...
                                // إذا كان هذا هو المشروع الأصلي واختيرت الدعوى الأصلية، قم بتحميل المكلفين
                                if (projectId == currentProjectId && lawsuitSelect.val() ==
                                    currentLawsuitId) {
                                    // تحميل المكلفين للدعوى المختارة أصلاً
                                    setTimeout(function() {
                                        $('#lawsuit_id').trigger('change');
                                    }, 100);
                                }
