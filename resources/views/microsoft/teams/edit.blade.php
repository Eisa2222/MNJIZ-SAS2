...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item d-flex align-items-center"><a href="#">تعديل الاجتماع</a>
        <i class="ti ti-star favorite-icon" data-page-name="تعديل بيانات الاجتماع" data-page-url="{{ url()->current() }}"
...
@section('content')
    ...
    <div class="card mb-6 p-7">
        <div class="card-body pt-4">
            <form id="meeting-form" class="needs-validation" novalidate
                action="{{ route('teams.update', $meeting->event_id) }}" method="POST">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="text" id="subject" name="subject" class="form-control"
                            value="{{ old('subject', $meeting->meeting_name) }}" required />
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="datetime-local" id="start_time" name="start_time" class="form-control"
                            value="{{ old('start_time', \Carbon\Carbon::parse($meeting->meeting_start_date)->format('Y-m-d\TH:i')) }}"
                            required />
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="datetime-local" id="end_time" name="end_time" class="form-control"
                            value="{{ old('end_time', \Carbon\Carbon::parse($meeting->meeting_end_date)->format('Y-m-d\TH:i')) }}"
                            required />
                        <div class="invalid-feedback">يرجى اختيار وقت انتهاء الاجتماع بعد وقت البدء.</div>
                    </div>

                    <!-- حقل اختيار أطراف الاجتماع -->
                    <div class="col-md-6 mb-4">
                        <label for="meeting_participants" class="form-label">أطراف الإجتماع</label>
                        <select id="meeting_participants" name="meeting_participants[]" class="form-control select2"
                            multiple="multiple" data-placeholder="اختر أطراف الإجتماع">
                            <option value="employees"
                                {{ in_array('employees', old('meeting_participants', $meeting->getParticipantTypes())) ? 'selected' : '' }}>
                                الموظفين</option>
                            <option value="customers"
                                {{ in_array('customers', old('meeting_participants', $meeting->getParticipantTypes())) ? 'selected' : '' }}>
                                العملاء</option>
                            <option value="opponents"
                                {{ in_array('opponents', old('meeting_participants', $meeting->getParticipantTypes())) ? 'selected' : '' }}>
                                الخصوم</option>
                            <option value="additional"
                                {{ in_array('additional', old('meeting_participants', $meeting->getParticipantTypes())) ? 'selected' : '' }}>
                                بريد آخر</option>
                        </select>
                        <div class="invalid-feedback">يرجى اختيار أطراف الاجتماع.</div>
                    </div>
                    ...
                    <div class="col-md-6 mb-4 hide-elements" id="field-employees">
                        ...
                        <select id="attendees_employee" name="attendees_employee[]" class="form-control select2"
                            ...
                            @foreach ($employees as $employee)
                                <option value="{{ $employee->work_email }}"
                                    {{ in_array($employee->work_email, old('attendees_employee', $attendees_employee)) ? 'selected' : '' }}>
                                    {{ $employee->name }} ({{ $employee->work_email }})
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select id="attendees_customer" name="attendees_customer[]" class="form-control select2"
                            ...
                            @foreach ($customers as $customer)
                                <option value="{{ $customer->email }}"
                                    {{ in_array($customer->email, old('attendees_customer', $attendees_customer)) ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 mb-4 hide-elements" id="field-opponents">
                        ...
                        <select id="attendees_opponent" name="attendees_opponent[]" class="form-control select2"
                            ...
                            @foreach ($opponents as $opponent)
                                <option value="{{ $opponent->email }}"
                                    {{ in_array($opponent->email, old('attendees_opponent', $attendees_opponent)) ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 mb-4 hide-elements" id="field-additional">
                        ...
                        <select id="additional_emails" name="additional_emails[]" class="form-control select2"
                            multiple="multiple" data-placeholder="أدخل بريد إلكتروني">
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <select id="meeting_field" name="meeting_field" class="form-select select2" required
                            ...
                            <option></option>
                            <option value="public"
                                {{ old('meeting_field', $meeting->meeting_field) == 'public' ? 'selected' : '' }}>عام
                            </option>

                            <option value="projects"
                                {{ old('meeting_field', $meeting->meeting_field) == 'projects' ? 'selected' : '' }}>
                                المشاريع
                            </option>
                        </select>
                    ...
                    <div class="col-md-6 mb-4 hide-elements" id="lawsuits_container">
                        ...
                        <select id="lawsuits_id" name="lawsuits_id" class="form-select select2"
                            ...
                            @foreach ($lawsuits as $lawsuit)
                                <option value="{{ $lawsuit->id }}"
                                    {{ old('lawsuits_id', $meeting->lawsuits_id) == $lawsuit->id ? 'selected' : '' }}>
                                    {{ $lawsuit->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-12 mb-4">
                        ...
                        <textarea id="meeting_points" name="meeting_points" class="form-control" rows="4">{{ old('meeting_points', $meeting->meeting_points) }}</textarea>
                        <div class="invalid-feedback">يرجى إدخال نقاط الاجتماع.</div>
                    </div>
                </div>

                <div class="d-flex justify-content-end">
                    <button type="submit" class="btn btn-primary">تحديث الاجتماع</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function fetchAndUpdateField(url, targetField, emptyMessage = "لا توجد بيانات", selectedValue = null) {
            ...
            // تهيئة المشاريع والدعاوى عند تحميل الصفحة
            const initialField = $('#meeting_field').val();
            const projectId = $('#project_id').val();
            const lawsuitId = $('#lawsuits_id').val();

            if (initialField === 'projects') {
                $('#projects_container').removeClass('hide-elements');
                if (projectId) {
            ...
            // تفعيل الحقول بناءً على القيم القديمة
            var initialParticipants = @json(old('meeting_participants', $meeting->getParticipantTypes()));
            Object.values(fields).forEach(function(fieldSelector) {
                var field = fieldSelector.replace('#field-', '');
                if (initialParticipants.includes(field)) {
                    $(fieldSelector).show();
                    $(fieldSelector).find('select').attr('required', 'required');
                }
            });
