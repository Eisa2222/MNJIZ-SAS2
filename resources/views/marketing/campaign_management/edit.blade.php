...
@section('title', 'تعديل بيانات الحملة')

@section('breadcrumb')
    <li><a href="#">التسويق</a></li>
    <li><a href="{{ route('marketing.campaign-management.index') }}">إدارة الحملات الإعلانية</a></li>
    <li class="breadcrumb-item d-flex align-items-center">
        <a href="#">تعديل بيانات الحملة</a>
        <i class="ti ti-star favorite-icon" data-page-name="تعديل بيانات الحملة" data-page-url="{{ url()->current() }}"
        ...
        <div class="col-12">
            <div id="wizard-validation" class="bs-stepper mt-2">
                    ...
                    <div class="step" data-target="#basic-info">
                        <button type="button" class="step-trigger">
                            ...
                            <span class="bs-stepper-label mt-1">
                                ...
                                <span class="bs-stepper-subtitle">تعديبل تفاصيل الحملة الإعلانية </span>
                            </span>
                        </button>
                    </div>
                </div>
                <div class="bs-stepper-content">
                    ...
                    @endif
                    <form id="form" action="{{ route('marketing.campaign-management.update',$campaign_management) }}" method="POST"
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="campaign_name" id="campaign_name" class="form-control"
                                        required value="{{ old('campaign_name',$campaign_management->campaign_name) }}">
                                </div>
                                    ...
                                    <select class="form-select select2" name="content_type_id" id="content_type_id"
                                        ...
                                        @foreach ($contentTypes as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('content_type_id',$campaign_management->content_type_id) == $item->id ? 'selected' : '' }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select select2" name="campaign_section_id" id="campaign_section_id"
                                        ...
                                        @foreach ($campaignSections as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('campaign_section_id',$campaign_management->campaign_section_id) == $item->id ? 'selected' : '' }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select select2" name="content_purpose_id" id="content_purpose_id"
                                        ...
                                        @foreach ($contentPurposes as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('content_purpose_id',$campaign_management->content_purpose_id) == $item->id ? 'selected' : '' }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select select2" name="social_id" id="social_id"
                                        ...
                                        @foreach ($socialPlatforms as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('social_id',$campaign_management->social_id) == $item->id ? 'selected' : '' }}>{{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="number" step="0.01" name="budget" id="budget" class="form-control"
                                        value="{{ old('budget',$campaign_management->budget) }}">
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" name="start_date" id="start_date" class="form-control"
                                        value="{{ old('start_date',$campaign_management->start_date) }}">
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" name="end_date" id="end_date" class="form-control"
                                        value="{{ old('end_date',$campaign_management->end_date) }}">
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select class="form-select select2" name="target_audience_id" id="target_audience_id"
                                        ...
                                        @foreach ($targetAudiences as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('target_audience_id',$campaign_management->target_audience_id) == $item->id ? 'selected' : '' }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea name="text" id="text" rows="3" class="form-control">{{ old('text',$campaign_management->text) }}</textarea>
                                </div>
                            </div>
