...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item d-flex align-items-center">
        <a href="#">تعديل بيانات المنتج</a>
        <i class="ti ti-star favorite-icon" data-page-name="تعديل بيانات المنتج" data-page-url="{{ url()->current() }}"
...
@section('page-script')
    ...
    <script>
        ...
        // القيمة المحفوظة لسبب الضريبة الخاصة
        const savedSpecialTaxReasonId = @json(old('special_tax_reason_id', $product['special_tax_reason_id'] ?? ''));

        const currentProductType = @json($product['type']);
    </script>
@endsection
        ...
        <div class="col-12">
            <div id="wizard-validation" class="bs-stepper mt-2">
                ...
                <div class="bs-stepper-content">
                    ...
                    <form id="form" action="{{ route('qoyod.products.update', $product['id']) }}" method="POST">
                        @csrf
                        @method('put')
                        ...
                        <div id="details-info" class="content dstepper-block">
                                    ...
                                    <select id="product_unit_type_id" name="product_unit_type_id"
                                        ...
                                        @foreach ($units as $unit)
                                            <option value="{{ $unit['id'] }}"
                                                {{ old('product_unit_type_id', $product['unit_type']) == $unit['id'] ? 'selected' : '' }}>
                                                {{ $unit['unit_name'] }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 d-none" id="wrapper_expense_account_id">
                                    ...
                                    <select id="expense_account_id" name="expense_account_id" class="select2 form-select"
                                        ...
                                        @foreach ($expenseAccount as $acc)
                                            <option value="{{ $acc['id'] }}"
                                                {{ old('expense_account_id', $product['expense_account_id']) == $acc['id'] ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6 d-none" id="wrapper_sales_account_id">
                                    ...
                                    <select id="sales_account_id" name="sales_account_id" class="select2 form-select"
                                        ...
                                        @foreach ($salesAccount as $acc)
                                            <option value="{{ $acc['id'] }}"
                                                {{ old('sales_account_id', $product['sales_account_id']) == $acc['id'] ? 'selected' : '' }}>
                                               {{ $acc['code'] }} - {{ $acc['name_ar'] }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-12 d-flex justify-content-between mt-4">
                                ...
                                <button type="submit" class="btn btn-primary btn-submit">
                                    <span class="d-sm-inline-block d-none me-sm-2">تحديث</span>
                                </button>
                            </div>
                        </div>
                    </form>
