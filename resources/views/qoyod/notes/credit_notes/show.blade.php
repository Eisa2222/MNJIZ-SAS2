...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item d-flex align-items-center">
        <a href="#"> تفاصيل إشعار دائن </a>
        <i class="ti ti-star favorite-icon" data-page-name="تفاصيل إشعار دائن" data-page-url="{{ url()->current() }}"
                ...
                <div class="card-body px-5">
                    <div class="table-responsive">
                        <table class="table table-striped  small text-center">
                            ...
                            <tr>
                                <td class="fw-bold">فاتورة المبيعات الأصلية</td>
                                <td>{{ $invoice['parent_id'] }}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td>{{ $invoice['inventory_name_from_relation'] }}</td>
                            </tr>
                            <tr>
                                <td class="fw-bold">سبب الإصدار</td>
                                <td>{{ $invoice['issuance_reason'] ?? '-' }}</td>
                            </tr>
                    ...
                    <ul class="timeline ">
                        <li class="timeline-item py-5">
                            ...
                            <div class="timeline-event">
                                ...
                                <small>
                                    <b>اضيف بواسطة</b>
                                    {{ empty($invoice['created_by']) ? 'غير محدد' : $invoice['created_by'] }}
                                </small>
                            </div>
                        </li>
                        <li class="timeline-item py-5">
                            ...
                            <div class="timeline-event">
                                ...
                                <small>
                                    <b> بواسطة</b>
                                    {{ empty($invoice['updated_by']) ? 'غير محدد' : $invoice['updated_by'] }}
                                </small>
                            </div>
                        </li>
                    </ul>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped small text-center">
                            ...
                            <tbody>
                                @foreach ($invoice['line_items'] as $index => $item)
                                    <tr>
                                        ...
                                        <td>
                                            {{ number_format($item['quantity'] * $item['price'] - $item['discount_amount'], 2) }}
                                        </td>
                            ...
                            <tfoot>
                                ...
                                <tr>
                                    <td colspan="4"></td>
                                    <td class="fw-bold">{{ number_format($invoice['total_discount'] ?? 0, 2) }} <span
                                            class="icon-saudi_riyal"></span></td>
                                    <td class="fw-bold">{{ number_format($invoice['subtotal'] ?? 0, 2) }} <span
                                            class="icon-saudi_riyal"></span></td>
                                    <td></td>
                                    <td class="fw-bold">{{ number_format($invoice['tax_amount'] ?? 0, 2) }} <span
                                            class="icon-saudi_riyal"></span></td>
                                    <td class="fw-bold">{{ number_format($invoice['total_amount'] ?? 0, 2) }} <span
                                            class="icon-saudi_riyal"></span></td>
                                </tr>
                            </tfoot>
                        </table>
        ...
        <div class="col-5">
            <div class="card p-5">
                <div class="d-flex justify-content-between py-2 border-bottom">
                    ...
                    <span class="fw-bold">{{ number_format($invoice['subtotal'] ?? 0, 2) }}
                        <span class="icon-saudi_riyal mx-2"></span>
                    </span>
                </div>
                <div class="d-flex justify-content-between py-2 border-bottom">
                    <span class="text-muted">قيمة الضريبة</span>
                    <span class="fw-bold">{{ number_format($invoice['tax_amount'] ?? 0, 2) }}
                        <span class="icon-saudi_riyal mx-2"></span>
                    </span>
                </div>
                <div class="d-flex justify-content-between py-3">
                    <span class="fw-bold text-primary ">المجموع النهائي</span>
                    <span class="fw-bold text-primary ">{{ number_format($invoice['total_amount'] ?? 0, 2) }}
                        <span class="icon-saudi_riyal mx-2"></span>
                    </span>
                </div>
            </div>
