...
@section('title', 'تفاصيل العرض السعري ')

@section('breadcrumb')
    <li><a href="#">قيود</a></li>
    <li><a href="{{ route('qoyod.quotes.index') }}">عروض الأسعار</a></li>
    <li class="breadcrumb-item d-flex align-items-center">
        <a href="#">تفاصيل العرض السعري </a>
        <i class="ti ti-star favorite-icon" data-page-name="تفاصيل العرض السعري" data-page-url="{{ url()->current() }}"
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped  small text-center">
                            <tr>
                                ...
                                <td>{{ $quotes['reference'] }}</td>
                            </tr>
                            <tr>
                                <td class="fw-bold">العميل</td>
                                <td>{{ $quotes['contact_name'] }}</td>
                            </tr>
                            <tr>
                                <td class="fw-bold">تاريخ الإصدار</td>
                                <td>{{ $quotes['issue_date'] }}</td>
                            </tr>
                            <tr>
                                <td class="fw-bold">تاريخ الإنتهاء</td>
                                <td>{{ $quotes['expiry_date'] }}</td>
                            </tr>
                            <tr>
                                <td class="fw-bold">الحالة</td>
                                <td>{{ $quotes['status'] == 'Approved' ? 'موافق عليه' : 'مسودة' }}</td>
                            </tr>
                            <tr>
                                <td class="fw-bold">الموقع</td>
                                <td>{{ $quotes['inventory_name_from_relation'] }}</td>
                            </tr>
                        ...
                        <li class="timeline-item py-5">
                            ...
                            <div class="timeline-event">
                                ...
                                <small class="text-muted d-block">
                                    {{ \Carbon\Carbon::parse($quotes['created_at'])->format('Y-m-d H:i') }}
                                </small>
                                <small>
                                    <b>اضيف بواسطة</b>
                                    {{ empty($quotes['created_by']) ? 'غير محدد' : $quotes['created_by'] }}
                                </small>
                            </div>
                        </li>
                        <li class="timeline-item py-5">
                            ...
                            <div class="timeline-event">
                                ...
                                <small class="text-muted d-block">
                                    {{ \Carbon\Carbon::parse($quotes['updated_at'])->format('Y-m-d H:i') }}
                                </small>
                                <small>
                                    <b> بواسطة</b>
                                    {{ empty($quotes['updated_by']) ? 'غير محدد' : $quotes['updated_by'] }}
                                </small>
                            </div>
                        </li>
                    </ul>
                ...
                <div class="card-body px-5">
                    <div class="table-responsive">
                        <table class="table table-striped small text-center">
                            ...
                            <tbody>
                                @foreach ($quotes['line_items'] as $index => $item)
                                    <tr>
                                        <td>{{ $index + 1 }}</td>
                            ...
                            <tfoot>
                                ...
                                <tr>
                                    <td colspan="4"></td>
                                    <td class="fw-bold">{{ number_format($quotes['total_discount'] ?? 0, 2) }} <span
                                            class="icon-saudi_riyal"></span></td>
                                    <td class="fw-bold">{{ number_format($quotes['subtotal'] ?? 0, 2) }} <span
                                            class="icon-saudi_riyal"></span></td>
                                    <td></td>
                                    <td class="fw-bold">{{ number_format($quotes['tax_amount'] ?? 0, 2) }} <span
                                            class="icon-saudi_riyal"></span></td>
                                    <td class="fw-bold">{{ number_format($quotes['total_amount'] ?? 0, 2) }} <span
                                            class="icon-saudi_riyal"></span></td>
                                </tr>
                            </tfoot>
                        </table>
        ...
        <div class="col-5">
            <div class="card p-5">
                <div class="d-flex justify-content-between py-2 border-bottom">
                    ...
                    <span class="fw-bold">{{ number_format($quotes['subtotal'] ?? 0, 2) }}
                        <span class="icon-saudi_riyal mx-2"></span>
                    </span>
                </div>
                <div class="d-flex justify-content-between py-2 border-bottom">
                    <span class="text-muted">قيمة الضريبة</span>
                    <span class="fw-bold">{{ number_format($quotes['tax_amount'] ?? 0, 2) }}
                        <span class="icon-saudi_riyal mx-2"></span>
                    </span>
                </div>
                <div class="d-flex justify-content-between py-3">
                    <span class="fw-bold text-primary ">المجموع النهائي</span>
                    <span class="fw-bold text-primary ">{{ number_format($quotes['total_amount'] ?? 0, 2) }}
                        <span class="icon-saudi_riyal mx-2"></span>
                    </span>
                </div>
            </div>
