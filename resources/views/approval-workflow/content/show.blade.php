...
@section('title', 'اعتماد محتوى - #' . $item->id)

@section('breadcrumb')
<li class="breadcrumb-item d-flex align-items-center">
    <a href="#">تفاصيل المحتوى #{{ $item->id }}</a>
</li>
@endsection

@section('page-script')
@vite(['resources/assets/js/approval-workflow/approval-config.js',
'resources/assets/js/approval-workflow/approval-manager.js', 'resources/assets/js/approval-workflow/content/show.js'])
    ...
    <div class="col-12 col-md-8 d-flex">
        <div class="card flex-fill d-flex flex-column h-100">
            <div class="card-header py-4">
                <h6 class="card-title mb-0"><i class="ti ti-file-text text-primary me-2"></i>تفاصيل المحتوى</h6>
            </div>
            <div class="border-1 border-light border-dashed mb-2"></div>
            <div class="card-body flex-fill overflow-auto">
                <div class="mb-3">
                    <h5 class="mb-2">نص المحتوى:</h5>
                    <p class="text-pre-wrap p-3 border rounded bg-light"
                        style="white-space: pre-wrap; word-wrap: break-word;">{{ $item->content_text ?? 'لا يوجد نص' }}
                    </p>
                </div>

                @if ($item->media_url)
                <div class="mb-3">
                    <h5 class="mb-2">المرفق المرئي:</h5>
                    @if ($item->media_url)
                    <div class="mb-4">
                        <h5 class="mb-2">المرفق المرئي:</h5>
                        @if ($item->isImage())
                        <a href="{{ Storage::url($item->media_url) }}" target="_blank" title="عرض الصورة بحجمها الكامل">
                            <img src="{{ Storage::url($item->media_url) }}" class="img-fluid rounded border"
                                alt="صورة المحتوى" style="max-height: 400px; cursor: pointer;">
                        </a>
                        @elseif($item->isVideo())
                        <video controls class="w-100 rounded border" style="max-height: 400px;">
                            <source src="{{ Storage::url($item->media_url) }}" type="video/mp4">
                ...
                <h5 class="mb-2 mt-4">بيانات إضافية:</h5>
                <div class="table-responsive rounded-3 border">
                    <table class="table table-striped table-hover mb-0">
                        <tbody>
                            <tr>
                                <td class="fw-bold text-muted py-3 ps-4 pe-3 bg-light" style="width: 35%;">نوع المحتوى
                                </td>
                                <td class="py-3 ps-3 pe-4 fw-semibold text-dark">
                                    {{ $item->contentType?->name ?? 'غير محدد' }}</td>
                            </tr>
                            <tr>
                                <td class="fw-bold text-muted py-3 ps-4 pe-3 bg-light">المنصة المستهدفة</td>
                                <td class="py-3 ps-3 pe-4 fw-semibold text-dark">
                                    {{ $item->social?->name ?? 'غير محدد' }}</td>
                            </tr>
                            <tr>
                                <td class="fw-bold text-muted py-3 ps-4 pe-3 bg-light">تاريخ النشر المقترح</td>
                                <td class="py-3 ps-3 pe-4 fw-semibold text-dark">
                                    {{ $item->publication_date?->format('d/m/Y') }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
