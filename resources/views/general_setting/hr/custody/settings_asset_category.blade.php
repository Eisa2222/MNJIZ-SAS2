...
@section('title', 'إعدادات تصنيفات الاصول')

@section('breadcrumb')
    <li class="breadcrumb-item d-flex align-items-center"><a href="#"> إعدادات تصنيفات الاصول
        </a>
        <i class="ti ti-star favorite-icon" data-page-name="إعدادات تصنيفات الاصول" data-page-url="{{ url()->current() }}"
...
@section('content')
            ...
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <div class="text-center mb-6">
                        <h4 class="mb-2">إضافة تصنيف جديد</h4>
                        <p> تصنيفات الاصول</p>
                    </div>
                    <form class="needs-validation row" novalidate id="" action="{{ route($route . '.store') }}"
                        ...
                        <div class="col-12 mb-4">
                            ...
                            <div class="invalid-feedback"> اسم التصنيف مطلوب
                            </div>
                        </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-simple">
            <div class="modal-content">
                <div class="modal-body">
                    <button type="button" class="btn-close btn-pinned" data-bs-dismiss="modal" aria-label="Close"></button>
                    <div class="text-center mb-6">
                        <h4 class="mb-2">تعديل بيانات التصنيف </h4>
                        <p> تصنيفات الاصول </p>
                    </div>
                    <form id="editSettingForm" class="needs-validation row" novalidate method="POST">
                        ...
                        @method('PUT')
                        <div class="col-12 mb-4">
                            <label class="form-label" for="modalSettingNameEdit">اسم التصنيف </label>
                            <input type="text" id="modalSettingNameEdit" name="name" class="form-control"
                                placeholder="اسم التصنيف " required />
                            <div class="valid-feedback"></div>
                            <div class="invalid-feedback">اسم التصنيف مطلوب</div>
                        </div>
                        <div class="col-12 text-center demo-vertical-spacing">
    ...
    <script>
        ...
        // التعامل مع تغيير التصنيف عند النقر على الشارة
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        toastr.success('تم تغيير التصنيف بنجاح.');
                    } else {
                        toastr.error('فشل في تغيير التصنيف.');
                    }
                },
                error: function(xhr) {
                    toastr.error('حدث خطأ أثناء تغيير التصنيف.');
                }
            });
        });
    </script>
