...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item d-flex align-items-center">
        <a href="#">تعديل نوع إجازة</a>
        <i class="ti ti-star favorite-icon" data-page-name="تعديل نوع إجازة" data-page-url="{{ url()->current() }}"
        ...
        <div class="col-12">
            <div id="wizard-validation" class="bs-stepper mt-2">
                ...
                <div class="bs-stepper-content">
                    ...
                    <form id="leave-type-form" action="{{ route('settings-leave-types.update', $leaveType->id) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" id="name" name="name" class="form-control"
                                        placeholder="أدخل اسم نوع الإجازة" value="{{ old('name', $leaveType->name) }}"
                                        required>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <select id="gender_applicability" name="gender_applicability"
                                        ...
                                        {{ old('is_global', $leaveType->is_global) ? 'disabled' : '' }}>
                                        <option value=""></option>
                                        <option value="both"
                                            {{ old('gender_applicability', $leaveType->gender_applicability) == 'both' ? 'selected' : '' }}>
                                            كلا الجنسين
                                        </option>
                                        <option value="female"
                                            {{ old('gender_applicability', $leaveType->gender_applicability) == 'female' ? 'selected' : '' }}>
                                            الإناث فقط
                                        </option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <select id="leave_unit_type" name="leave_unit_type" class="form-select select2"
                                        ...
                                        <option value="full_day"
                                            {{ old('leave_unit_type', $leaveType->leave_unit_type ?? 'full_day') == 'full_day' ? 'selected' : '' }}>
                                            أيام كاملة فقط
                                        </option>
                                        <option value="half_day"
                                            {{ old('leave_unit_type', $leaveType->leave_unit_type ?? '') == 'half_day' ? 'selected' : '' }}>
                                            نصف يوم
                                        </option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <input type="number" id="advance_notice_days" name="advance_notice_days"
                                        ...
                                        value="{{ old('advance_notice_days', $leaveType->advance_notice_days ?? 0) }}">
                                </div>
                                <div class="col-12">
                                ...
                                <div class="col-12">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="is_global" name="is_global"
                                            value="1"
                                            {{ old('is_global', $leaveType->is_global ?? false) ? 'checked' : '' }}>
                                ...
                                <div class="col-12">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="count_weekends"
                                            ...
                                            {{ old('count_weekends', $leaveType->count_weekends ?? false) ? 'checked' : '' }}>
                        ...
                        <div id="step-conditions" class="content">
                            @php
                                $isGlobal = old('is_global', $leaveType->is_global ?? false);
                            @endphp
                            <div class="row g-3">
                                <div class="col-12">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="is_deductible"
                                            ...
                                            {{ old('is_deductible', $leaveType->is_deductible) ? 'checked' : '' }}>
                                ...
                                <div class="col-12">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="is_carry_forwardable"
                                            ...
                                            {{ old('is_carry_forwardable', $leaveType->is_carry_forwardable ?? false) ? 'checked' : '' }}>
                                ...
                                <div id="dateFields" class="row g-3"
                                    style="{{ $isGlobal ? 'display:flex;' : 'display:none;' }}">
                                    <div class="col-md-4">
                                        ...
                                        <input type="date" id="start_date" name="start_date" class="form-control"
                                            ...
                                            value="{{ old('start_date', optional($leaveType->start_date)->format('Y-m-d')) }}">
                                    </div>
                                    <div class="col-md-4">
                                        ...
                                        <input type="date" id="end_date" name="end_date" class="form-control"
                                            min="{{ now()->format('Y-m-d') }}"
                                            value="{{ old('end_date', optional($leaveType->end_date)->format('Y-m-d')) }}">
                                    </div>
                                    <div class="col-md-4">
                                        ...
                                        <input type="text" id="daysCalculated" class="form-control" readonly disabled
                                            value="{{ $isGlobal && old('start_date', $leaveType->start_date) && old('end_date', $leaveType->end_date)
                                                ? \Carbon\Carbon::parse(old('end_date', $leaveType->end_date))->diffInDays(
                                                        \Carbon\Carbon::parse(old('start_date', $leaveType->start_date)),
                                                    ) + 1
                                                : '' }}">
                                    </div>
                                </div>

                                {{-- حقل مخفي لحفظ قيمة الأيام --}}
                                <input type="hidden" id="days_hidden" name="days"
                                    value="{{ old('days', $leaveType->days) }}" disabled>

                                {{-- الحد الأقصى للأيام (للإجازات العادية فقط) --}}
                                <div id="manualDays" class="row g-3"
                                    style="{{ $isGlobal ? 'display:none;' : 'display:flex;' }}">
                                    <div class="col-md-6">
                                        ...
                                        <input type="number" id="days" name="days" class="form-control"
                                            min="0" value="{{ old('days', $leaveType->days) }}"
                                            {{ !$isGlobal ? 'required' : '' }}>
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <input type="number" id="max_requests" name="max_requests" class="form-control"
                                            min="0"
                                            value="{{ old('max_requests', $leaveType->max_requests ?? 0) }}">
                                    </div>
                                </div>
                                {{-- عتبة سنوات الخدمة --}}
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="number" id="service_years_threshold" name="service_years_threshold"
                                            class="form-control" min="0"
                                            value="{{ old('service_years_threshold', $leaveType->service_years_threshold ?? 0) }}">
                                    </div>
                                    <div class="col-md-6">
                                        ...
                                        <input type="number" id="days_after_threshold" name="days_after_threshold"
                                            class="form-control" min="0"
                                            value="{{ old('days_after_threshold', $leaveType->days_after_threshold ?? 0) }}">
                                    </div>
                                </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="number" id="min_service_years" name="min_service_years"
                                        class="form-control" min="0"
                                        value="{{ old('min_service_years', $leaveType->min_service_years) }}">
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-12">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="has_attachments"
                                            ...
                                            {{ old('has_attachments', $leaveType->has_attachments ?? false) ? 'checked' : '' }}>
                                ...
                                <div id="attachmentDescRow" class="col-12"
                                    style="{{ old('has_attachments', $leaveType->has_attachments ?? false) ? 'display:block;' : 'display:none;' }}">
                                    ...
                                    <input type="text" id="attachment_description" name="attachment_description"
                                        ...
                                        value="{{ old('attachment_description', $leaveType->attachment_description ?? '') }}">
                                </div>
                            </div>
                            <div class="d-flex justify-content-between mt-4">
                                <button type="button" class="btn btn-label-secondary btn-prev">
