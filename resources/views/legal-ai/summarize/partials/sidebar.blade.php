    ...
    <div class="sidebar-body">
        <div class="d-flex justify-content-between align-items-center px-3 py-2 border-bottom flex-shrink-0">
            <h5 class="text-primary mb-0 d-flex align-items-center">
                <span>سجل التلخيص</span>
            </h5>
            <button id="new-summary-btn" class="btn btn-sm btn-primary" title="بدء جلسة تلخيص جديدة">
        ...
        <div class="chat-list-scroll-container">
            <ul class="list-unstyled chat-contact-list py-2 mb-0" id="chat-list">
                            ...
                            <div class="chat-contact-info flex-grow-1 overflow-hidden">
                                ...
                                {{-- @if ($lastMessage = $c->messages->last())
                                    <small class="chat-contact-status text-truncate text-muted">
                                        {{ Str::limit(strip_tags(str_replace(['**', '_'], '', $lastMessage->message)), 25) }}
                                    </small>
                                @else
                                    <small class="chat-contact-status text-truncate text-muted">لم يبدأ التلخيص
                                        بعد</small>
                                @endif --}}
                            </div>
                        </a>
                ...
                @empty
                    <li class="text-center p-4 text-muted">لا توجد ملخصات سابقة.</li>
                @endforelse
            </ul>
        </div>
    </div>
