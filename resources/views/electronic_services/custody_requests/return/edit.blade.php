        ...
        <div class="col-12">
            <div id="wizard-validation" class="bs-stepper mt-2">
                ...
                <div class="bs-stepper-content">
                    <form id="form"
                        action="{{ route('account.electronic-services.custody-requests.return.update', $custody_request->id) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select id="parent_request_id" name="parent_request_id"
                                        ...
                                        @foreach ($assignRequests as $req)
                                            <option value="{{ $req->id }}"
                                                {{ old('parent_request_id', $custody_request->parent_request_id) == $req->id ? 'selected' : '' }}>
                                                {{ $req->item->name }}
                                    ...
                                    <select id="return_status" name="return_status" class="form-select select2"
                                        ...
                                        @foreach ($statusOptions as $type)
                                            <option value="{{ $type['id'] }}"
                                                {{ old('return_status', $custody_request->return_status->value) == $type['id'] ? 'selected' : '' }}>
                                                {{ $type['name'] }}
                                            </option>
                                        @endforeach
                                    </select>
