...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item d-flex align-items-center">
        <a href="#">تفاصيل المكافأة</a>
        <i class="ti ti-star favorite-icon" data-page-name="تفاصيل المكافأة" data-page-url="{{ url()->current() }}"
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped small text-center">
                            <tr>
                                ...
                                <td>{{ $reward->reward_number }}</td>
                            </tr>
                            <tr>
                                <td class="fw-bold">الموظف</td>
                                <td>
                                    <a href="{{ route('account.employee.profile', $reward->employee_id) }}">
                                        {{ $reward->employee->getRawNameAttribute() }}
                                    </a>
                                </td>
                            </tr>
                            <tr>
                                <td class="fw-bold">نوع المكافأة</td>
                                <td>{{ $reward->reward_type->label() }}</td>
                            </tr>
                            <tr>
                                <td class="fw-bold">تاريخ المكافأة</td>
                                <td>{{ $reward->reward_date }}</td>
                            </tr>
                            <tr>
                                <td class="fw-bold">المبلغ</td>
                                <td>{{ number_format($reward->amount, 2) }} <span class="icon-saudi_riyal"></span></td>
                            </tr>
                            <tr>
                        ...
                        <div class="card-body d-flex align-items-center justify-content-center">
                            <div class="text-center">
                                ...
                                @if ($reward->updated_at)
                                    <small
                                        class="text-muted d-block">{{ $reward->updated_at->format('d/m/Y H:i') }}</small>
                                @endif
                            </div>
                        </div>
                    </div>
                        ...
                        <ul class="timeline">
                            <li class="timeline-item">
                                ...
                                <div class="timeline-event">
                                    ...
                                    <small class="text-muted d-block">{{ $reward->created_at }}</small>
                                    <small>
                                        <b>اضيف بواسطة</b>
                                        <a href="{{ route('account.employee.profile', $reward->created_by) }}">
                                            {{ $reward->createdBy->getRawNameAttribute() }}
                                        </a>
                                    </small>
                                </div>
                            </li>

                            @if ($reward->updated_by)
                                <li class="timeline-item">
                                    <span class="timeline-point bg-success"></span>
                                    <div class="timeline-event">
                                        <small class="timeline-title text-capitalize">تم التعديل بتاريخ</small>
                                        <small class="text-muted d-block">{{ $reward->updated_at }}</small>
                                        <small>
                                            <b>التعديل بواسطة</b>
                                            <a href="{{ route('account.employee.profile', $reward->updated_by) }}">
                                                {{ $reward->updatedBy->getRawNameAttribute() }}
                                            </a>
                                        </small>
                                    </div>
                                </li>
