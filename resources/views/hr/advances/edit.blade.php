...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item d-flex align-items-center">
        <a href="#">تعديل سلفة</a>
        <i class="ti ti-star favorite-icon" data-page-name="تعديل سلفة" data-page-url="{{ url()->current() }}"
...
@section('page-script')
    @vite(['resources/assets/js/hr/advances/advances-request-wizard.js'])
@endsection
        ...
        <div class="col-12">
            <div id="wizard-validation" class="bs-stepper mt-2">
                    ...
                    <div class="step" data-target="#basic-info">
                        <button type="button" class="step-trigger">
                            ...
                            <span class="bs-stepper-label mt-1">
                                ...
                                <span class="bs-stepper-subtitle">تفاصيل السلفة </span>
                            </span>
                        </button>
                    </div>
                </div>
                <div class="bs-stepper-content">
                    ...
                    @endif
                    <form id="form" action="{{ route('hr.advances.update', $advance->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div id="basic-info" class="content dstepper-block">
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    <label for="advance_type" class="form-label">نوع السلفة</label>
                                    <select id="advance_type" name="advance_type" class="form-select select2"
                                        data-placeholder="اختر نوع السلفة" required>
                                        <option value="">اختر النوع</option>
                                        @foreach ($advanceTypes as $type)
                                            <option value="{{ $type['id'] }}"
                                                {{ old('advance_type', $advance->advance_type->value) == $type['id'] ? 'selected' : '' }}>
                                                {{ $type['name'] }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    <label for="advance_date" class="form-label">تاريخ السلفة
                                        <span class="text-danger">*</span>
                                    </label>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="date" id="due_date" name="due_date" class="form-control"
                                        value="{{ old('due_date', optional($advance->due_date)->format('Y-m-d')) }}">
                                </div>

                                <div class="col-md-12 mb-3">
                                    <label for="notes" class="form-label">ملاحظات</label>
                                    <textarea id="notes" name="notes" class="form-control" rows="3">{{ old('notes', $advance->notes) }}</textarea>
                                </div>

                            </div>

                            <div class="d-flex justify-content-between mt-4">
                                <a href="{{ route('hr.advances.index') }}" class="btn btn-secondary">إلغاء</a>
