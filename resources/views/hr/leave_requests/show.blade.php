...
<li><a href="{{ route('hr.leave-requests.index') }}">طلبات الإجازات</a></li>
                    ...
                    <table class="table table-striped table-hover mb-0">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="py-3 ps-3 pe-4 fw-semibold text-dark">
                                    @if($leaveRequest->days_count)
                                    @if(is_int($leaveRequest->days_count))
                                    {{ $leaveRequest->days_count }}
                                    @else
                                    {{ number_format($leaveRequest->days_count, 1) }}
                                    @endif
                                    يوم
                                    @else
                                    غير محدد
                            ...
                            @if($leaveRequest->start_datetime || $leaveRequest->end_datetime)
                            <tr>
                                <td class="fw-bold text-muted py-3 ps-4 pe-3 bg-light">وقت البداية</td>
                                <td class="py-3 ps-3 pe-4 fw-semibold text-dark">
                                    {{ $leaveRequest->start_datetime ?
                                    \Carbon\Carbon::parse($leaveRequest->start_datetime)->format('H:i') : 'غير محدد' }}
                                </td>
                            </tr>
                            <tr>
                                <td class="fw-bold text-muted py-3 ps-4 pe-3 bg-light">وقت النهاية</td>
                                <td class="py-3 ps-3 pe-4 fw-semibold text-dark">
                                    {{ $leaveRequest->end_datetime ?
                                    \Carbon\Carbon::parse($leaveRequest->end_datetime)->format('H:i') : 'غير محدد' }}
                                </td>
                            </tr>
                            @endif
                            @if($leaveRequest->hours_count)
                            <tr>
                                <td class="fw-bold text-muted py-3 ps-4 pe-3 bg-light">عدد الساعات</td>
                                <td class="py-3 ps-3 pe-4 fw-semibold text-dark">
                                    {{ $leaveRequest->hours_count }} {{ $leaveRequest->hours_count == 1 ? 'ساعة' :
                                    'ساعات' }}
                                </td>
                            </tr>
                            @endif
                            <tr>
                                ...
                                <td class="py-3 ps-3 pe-4">
                                    @switch($leaveRequest->status->value ?? $leaveRequest->status)
                                    @case('pending')
                                    <span class="badge bg-warning fw-semibold">قيد الانتظار</span>
                                    @break
                                    @case('approved')
                                    <span class="badge bg-success fw-semibold">موافَق عليه</span>
                                    @break
                                    @case('rejected')
                                    <span class="badge bg-danger fw-semibold">مرفوض</span>
                                    @break
                                    @case('closed')
                                    <span class="badge bg-secondary fw-semibold">مغلق</span>
                                    @break
                                    @default
                                    <span class="badge bg-secondary fw-semibold">غير معروف</span>
                                    @endswitch
                                </td>
                            </tr>
                            <tr>
                        ...
                        <div class="d-flex align-items-center p-3 border rounded hover-shadow">
                            ...
                            <div class="flex-grow-1">
                                <h6 class="mb-1 text-truncate"
                                    title="{{ $attachment->file_name ?? $attachment->original_name ?? 'مرفق' }}">
                                    {{ Str::limit($attachment->file_name ?? $attachment->original_name ?? 'مرفق', 25) }}
                                </h6>
        ...
        <div class="card shadow-sm mb-3 flex-fill">
            ...
            <div class="card-body d-flex flex-column flex-fill overflow-auto p-0">
                ...
                <div class="p-4">
                    ...
                    @php
                    $statusConfig = $stage['config']; // جاهز من Backend!
        ...
        <div class="card shadow-sm flex-fill">
            ...
            <div class="card-body flex-fill d-flex flex-column">
                <ul class="timeline ms-2 flex-grow-1">
                    ...
                    <li class="timeline-item timeline-item-transparent pb-4">
                        ...
                        <div class="timeline-event">
                            ...
                            <p class="mb-0 text-muted">
                                <strong>بواسطة:</strong> {{ $leaveRequest->createdBy->name ??
                                $leaveRequest->employee->name ?? 'غير معروف' }}
                            </p>
                        </div>
                    </li>
                    ...
                    <!-- آخر تحديث -->
                    @php
                    $hasUpdates = $leaveRequest->updated_at && $leaveRequest->updated_at->gt($leaveRequest->created_at);
                    $showStatusChange = !in_array($leaveRequest->status->value ?? $leaveRequest->status, ['pending']);
                    @endphp
                    ...
                    <li class="timeline-item timeline-item-transparent">
                        <span class="timeline-point bg-{{ $statusConfig['color'] }}"></span>
                        <div class="timeline-event">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                <h6 class="timeline-title mb-0">تم {{ $statusConfig['label'] }}</h6>
                                <small class="text-muted">{{ $leaveRequest->updated_at?->format('d/m/Y H:i') }}</small>
                            </div>
                            <div class="alert {{ $statusConfig['alert_class'] }} p-2 small mb-0">
