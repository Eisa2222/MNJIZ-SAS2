...
function initializeEditForm() {
    ...
    if (currentData.locationId) {
        $('#location_id').val(currentData.locationId).trigger('change');
    }

    // تحميل العهد مع العهدة الحالية
    if (currentData.categoryId && currentData.locationId) {
        setTimeout(function () {
...
function bindFilterEvents() {
    $('#category_id, #location_id').on('change', function () {
        ...
        if (categoryId === currentData.categoryId && locationId === currentData.locationId && $('#custody_item_id option[value="' + currentData.custodyItemId + '"]').length > 0) {
            return;
        }
